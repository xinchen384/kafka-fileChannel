...
import org.apache.kafka.common.network.{TransportLayer, MyFileChannel}
...
class FileMessageSet private[kafka](@volatile var file: File,
                                    private[log] val channel: MyFileChannel,
  ...
  if (!isSlice){
    ...
    println( "the size of channel: " + channel.size().toInt + "  the end: " + end )
   ...
   */
  def this(file: File, channel: MyFileChannel) =
   ...
   */
  def this(file: File, channel: MyFileChannel, start: Int, end: Int) =
  ...
  def searchFor(targetOffset: Long, startingPosition: Int): OffsetPosition = {
    ...
    while(position + MessageSet.LogOverhead < size) {
      buffer.rewind()
      println("XIN in FileSegment position: " + position + " totalSize: " + size)
      channel.read(buffer, position)
      if(buffer.hasRemaining)
        throw new IllegalStateException("Failed to read complete buffer for targetOffset %d startPosition %d with %d size in %s"
                                        .format(targetOffset, startingPosition, size, file.getAbsolutePath))
      buffer.rewind()
  ...
  //def writeTo(destChannel: MyFileChannel, writePosition: Long, size: Int): Int = {
  ...
  def writeToMy(destChannel: MyFileChannel, writePosition: Long, size: Int): Int = {
    // Ensure that the underlying size has not changed.
    println("XIN this is not supposed to be called!!! ")
    0
  }
  /**
  ...
  def append(messages: ByteBufferMessageSet) {
    val written = messages.writeToMy(channel, 0, messages.sizeInBytes)
...
object FileMessageSet
{
  ...
  def openChannel(file: File, mutable: Boolean, fileAlreadyExists: Boolean = false, initFileSize: Int = 0, preallocate: Boolean = false): MyFileChannel = {
    val myChannel = new MyFileChannel()
    myChannel.openChannel(file, mutable, fileAlreadyExists, initFileSize, preallocate)
    myChannel
    /*
    if (mutable) {
