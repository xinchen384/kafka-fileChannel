...
ssh d0101b02 "sh /homes/hny9/chenxin/kafka/modified-kafka/bin/kafka-run-class.sh org.apache.kafka.tools.ProducerPerformance --latency 0 --topic ttt --num-records $numRecord --record-size $recordSize --throughput -1 --producer-props acks=1 bootstrap.servers=10.6.37.24:9092 buffer.memory=67108864 batch.size=32768 >& /homes/hny9/chenxin/kafka/modified-kafka/benchmark/logs/producer.log" & 
...
ssh d0101b03 "sh /homes/hny9/chenxin/kafka/modified-kafka/bin/kafka-consumer-perf-test.sh --new-consumer true --zookeeper 10.6.37.24:2181 --broker-list 10.6.37.24:9092 --messages $numRecord --topic ttt --threads 1 --group 1 >& /homes/hny9/chenxin/kafka/modified-kafka/benchmark/logs/consumer1.log 2>&1" 

#another consumer
#ssh d0101b04 "sh /homes/hny9/chenxin/kafka/modified-kafka/bin/kafka-consumer-perf-test.sh --new-consumer true --zookeeper 10.6.37.24:2181 --broker-list 10.6.37.24:9092 --messages $numRecord --topic ttt --threads 1 --group 2 >& /homes/hny9/chenxin/kafka/modified-kafka/benchmark/logs/consumer2.log 2>&1"
